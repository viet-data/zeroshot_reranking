...
class InContextReranker():
    ...
    def gather(self, states, length):
        states = states[0]
        last = states[length:].mean(dim=0, keepdim=True)
    ...
    def get_state(self, doc, dct):
        ...
        with torch.no_grad():
            ...
            old_values = torch.cat([(i[:original_length].mean(dim=1, keepdim=True) + self.gather(i, original_length))/2 for i in old_values], dim=0)
            dct[doc] = old_values
    ...
    def _prepare_input_for_document_retrieval(self, query, documents, system_prompt='', query_position='last'):
        ...
        sample_answer = chat_with_llama(f"Query", self.llm, self.tokenizer)
        print(sample_answer)
        llm_prompt = ''
        document_span_intervals = []
