        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            if isinstance(past_key_value, DynamicCacheWithQuery):
                ...
                # For other general usage, such as generation
...
class LlamaFlashAttention2(LlamaAttention):
    ...
    def forward(
        ...
        key_states = self.k_proj(hidden_states)
        value_states = self.v_proj(hidden_states)
        # print('--> query_states: {}'.format(query_states.size()))
        # print('--> key_states: {}'.format(key_states.size()))
        # print('--> value_states: {}'.format(value_states.size()))
        ...
        if past_key_value is not None:
            ...
            if isinstance(past_key_value, DynamicCacheWithQuery):
                # print("Using DynamicCacheWithQuery")
                # For use in IC-RAG
                query_states_to_cache = query_states[:, :, past_key_value._query_indices, :]
                key_states, value_states = past_key_value.update(query_states_to_cache, key_states, value_states, self.layer_idx, cache_kwargs)
            else:
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            try:
                if embeddings is not None:
                    new_embeddings = torch.cat([hidden_states[0][input_ids[0]==self.tokenzier.bos_token_id][:1,:], embeddings[idx].to(hidden_states.device)], dim=0)
